...
public class DiscoveryExample {
    ...
    private static final String PATH = "/discover/example";
    ...
    private static void processCommands(ServiceDiscovery<InstanceDetails> serviceDiscovery, Map<String, ServiceProvider<InstanceDetails>> providers, CuratorFramework client) throws Exception {
        printHelp();
        List<DiscoveryServer> servers = Lists.newArrayList();
        try {
    ...
    private static void deleteInstance(String[] args, String command, List<DiscoveryServer> servers) {
    ...
    private static void addInstance(String[] args, CuratorFramework client, String command, List<DiscoveryServer> servers) throws Exception {
        if (args.length < 2) {
        ...
        String serviceName = args[0];
        DiscoveryServer server = new DiscoveryServer(client, PATH, serviceName, description.toString());
        servers.add(server);
        server.start();
